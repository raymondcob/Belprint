...
const ExpandedComponent = ({ data }) => {
  const subtotal = data.items.reduce((sum, item) => sum + item.qty * item.unitCost, 0);
  const gst = subtotal * 0.125;
  const total = subtotal + gst;

  return (
    <div className="p-4 rounded-xl bg-gray-100 flex justify-center">
      <div className="w-full max-w-4xl rounded-lg bg-white p-6 shadow-lg border-2 border-gray-300">
        ...
        <ExpandedQuoteItems items={data.items} />
        ...
        <div className="text-right mt-6">
          <button onClick={() => window.print()} className="flex items-center gap-3 px-6 py-3 rounded-lg border-2 border-blue-600 text-blue-600 font-medium hover:bg-blue-600 hover:text-white transition-colors">
            <FaPrint /> Print
          </button>
        </div>
      </div>
