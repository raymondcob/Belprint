...
const QuotePDFView = () => {
  ...
  // Calculate totals (you can reuse the logic from ExpandedComponent)
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {quoteData.items.map(item => (
              <tr key={item.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${(item.qty * item.unitCost).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
